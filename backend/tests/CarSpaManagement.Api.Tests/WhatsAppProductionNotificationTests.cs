...
public class WhatsAppProductionNotificationTests
{
    ...
    private static string EnInvoiceTemplateWithDocumentDiscoveryJson()
    {
        return JsonSerializer.Serialize(new
        {
            data = new object[]
            {
                new
                {
                    ...
                    components = new object[]
                    {
                        ...
                        {
                            type = "BODY",
                            text = "Hello {{1}},\n\nYour invoice {{2}} for your vehicle {{3}} total {{4}} is ready.",
                            example = new
                            {
                                body_text = new[]
                                {
    ...
    [Fact]
    public async Task ProcessMessage_WhenConfiguredWithEnUs_FindsMatchingEnUsTemplate()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        // Assert
        Assert.True(result);
        Assert.Equal("en_US", capturedLanguage);
    }

    [Fact]
    public async Task QueueInvoiceFinalized_StoresFourCoreParametersInJson()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        Assert.True(root.TryGetProperty("parameters", out var paramsProp));
    ...
    [Fact]
    public async Task ProcessMessage_WhenTemplateContainsUrlButton_ResolvesButtonAndSendsSuccessfully()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
            ...
            if (uri.Contains("/messages") && req.Content != null)
            {
                capturedMessagesPayload = req.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("{\"messages\":[{\"id\":\"wamid.BUTTON_TEST_OK\"}]}", Encoding.UTF8, "application/json") };
            }
        ...
        // Assert: Succeeded without requiring document header or PDF media upload
        Assert.True(result);
        var refreshed = await db.WhatsAppMessages.FindAsync(msg.Id);
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Sent, refreshed.Status);
        Assert.Equal("wamid.BUTTON_TEST_OK", refreshed.MetaMessageId);

        // Verify no /media upload request occurred
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/media"));

        // Verify message payload components: 1 body component + 1 button component
        ...
        Assert.Equal(4, bodyComp.GetProperty("parameters").GetArrayLength());

        var btnComp = components[1];
        Assert.Equal("button", btnComp.GetProperty("type").GetString());
        Assert.Equal("url", btnComp.GetProperty("sub_type").GetString());
        Assert.Equal("0", btnComp.GetProperty("index").GetString());
        var btnToken = btnComp.GetProperty("parameters")[0].GetProperty("text").GetString();
        Assert.False(string.IsNullOrWhiteSpace(btnToken));
    }

    [Fact]
    public async Task ProcessMessage_WhenTemplateHasDocumentHeader_GeneratesPdfAndAttachesDocumentHeader()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
            ...
            if (uri.Contains("message_templates"))
            {
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(EnInvoiceTemplateWithDocumentDiscoveryJson(), Encoding.UTF8, "application/json") };
            }
            if (uri.Contains("/media"))
            {
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("{\"id\":\"media_pdf_123\"}", Encoding.UTF8, "application/json") };
            }
            if (uri.Contains("/messages") && req.Content != null)
            {
                capturedMessagesPayload = req.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("{\"messages\":[{\"id\":\"wamid.PDF_DOC_OK\"}]}", Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.NotFound);
        };
        ...
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Sent, refreshed.Status);
        Assert.Equal("wamid.PDF_DOC_OK", refreshed.MetaMessageId);

        // Verify media upload occurred
        Assert.Contains(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/media"));

        // Verify payload has document header
        Assert.NotNull(capturedMessagesPayload);
        using var doc = JsonDocument.Parse(capturedMessagesPayload);
        var components = doc.RootElement.GetProperty("template").GetProperty("components");
        var headerComp = components[0];
        Assert.Equal("header", headerComp.GetProperty("type").GetString());
        Assert.Equal("document", headerComp.GetProperty("parameters")[0].GetProperty("type").GetString());
        Assert.Equal("media_pdf_123", headerComp.GetProperty("parameters")[0].GetProperty("document").GetProperty("id").GetString());
    }

    [Fact]
    public async Task ProcessMessage_WhenTemplateIsTextOnly_SendsSuccessfullyWithoutDocumentHeader()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
                    ...
                    data = new object[]
                    {
                        new
                        {
                            ...
                            components = new object[]
                            {
                                ...
                                {
                                    type = "BODY",
                                    text = "Hello {{1}}, your invoice {{2}} for vehicle {{3}} total {{4}} is ready."
                                }
                            }
                        }
                    }
            ...
            {
                capturedMessagesPayload = req.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("{\"messages\":[{\"id\":\"wamid.TEXT_ONLY_OK\"}]}", Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.NotFound);
        };
        ...
        // Assert: Succeeded without requiring document header
        Assert.True(result);
        var refreshed = await db.WhatsAppMessages.FindAsync(msg.Id);
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Sent, refreshed.Status);
        Assert.Equal("wamid.TEXT_ONLY_OK", refreshed.MetaMessageId);

        // Verify no /media upload request occurred
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/media"));

        // Verify payload has only body component (no header, no button)
        Assert.NotNull(capturedMessagesPayload);
        using var doc = JsonDocument.Parse(capturedMessagesPayload);
        var components = doc.RootElement.GetProperty("template").GetProperty("components");
    ...
    [Fact]
    public async Task ProcessMessage_WhenTemplateContainsUnsupportedComponent_FailsSafelyWithoutCallingMeta()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
            ...
            if (uri.Contains("message_templates"))
            {
                var unsupportedDiscovery = JsonSerializer.Serialize(new
                {
                    data = new object[]
                    {
                        new
                        {
                            ...
                            category = "UTILITY",
                            language = "en",
                            id = "tpl_inv_unsupported",
                            components = new object[]
                            {
                                new { type = "BODY", text = "Hello {{1}}" },
                                new { type = "LOCATION" } // Unsupported component type
                            }
                        }
                    }
                });
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(unsupportedDiscovery, Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.OK);
        };
        ...
        // Assert: Rejected before calling Meta messages API
        Assert.False(result);
        var refreshed = await db.WhatsAppMessages.FindAsync(msg.Id);
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Failed, refreshed.Status);
        Assert.Contains("unsupported components", refreshed.ErrorMessage);
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/messages"));
    }

    [Fact]
    public async Task ProcessMessage_WhenIncorrectParameterCount_FailsWithDescriptiveError()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
            ...
            if (uri.Contains("message_templates"))
            {
                var paramMismatchDiscovery = JsonSerializer.Serialize(new
                {
                    data = new object[]
                    {
                        new
                        {
                            ...
                            components = new object[]
                            {
                                ...
                                {
                                    type = "BODY",
                                    text = "Var1: {{1}}, Var2: {{2}}, Var3: {{3}}, Var4: {{4}}, Var5: {{5}}, Var6: {{6}}, Var7: {{7}}, Var8: {{8}}"
                                }
                            }
                        }
                    }
                });
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(paramMismatchDiscovery, Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.OK);
        };
        ...
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Failed, refreshed.Status);
        Assert.Contains("requires 8 BODY variable(s)", refreshed.ErrorMessage);
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/messages"));
    }

    [Fact]
    public async Task ProcessMessage_WhenTemplateLanguageMismatch_FailsSafely()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Failed, refreshed.Status);
        Assert.Contains("Template language mismatch", refreshed.ErrorMessage);
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/messages"));
    }
    ...
    [Fact]
    public async Task ProcessMessage_PaymentCompleted_WithUrlButtonTemplate_SendsSuccessfully()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        handler.ResponseFactory = req =>
        {
            ...
            {
                capturedMessagesPayload = req.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("{\"messages\":[{\"id\":\"wamid.PAY_BTN_OK\"}]}", Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.OK);
        };
        ...
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Sent, refreshed.Status);
        Assert.Equal("wamid.PAY_BTN_OK", refreshed.MetaMessageId);
    ...
    [Fact]
    public async Task ProcessMessage_WhenMissingInvoicePublicLinkToken_FailsDescriptively()
    {
        // Arrange
        using var db = CreateInMemoryDbContext();
        var enc = CreateEncryptionService();
        ...
        var msg = new WhatsAppMessage
        {
            ...
            TemplateParametersJson = "{\"parameters\":[\"Customer\",\"INV-001\",\"TN01AB1234\",\"1500\"]}",
            CreatedAt = DateTime.UtcNow
        };
        ...
        Assert.NotNull(refreshed);
        Assert.Equal(WhatsAppMessageStatus.Failed, refreshed.Status);
        Assert.Contains("requires a dynamic URL button parameter", refreshed.ErrorMessage);
        Assert.DoesNotContain(handler.RecordedRequests, r => r.RequestUri!.ToString().Contains("/messages"));
    }
