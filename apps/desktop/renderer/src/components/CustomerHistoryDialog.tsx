...
import { getJobCards, getVehiclesByCustomer, type CustomerDto, type JobCardDto, type VehicleDto } from '../lib/api';

const STATUS_MAP: Record<string, { bg: string; text: string; border: string }> = {
 Draft: { bg: '#fef3c7', text: '#92400e', border: '#fde047' },
 InProgress: { bg: '#dbeafe', text: '#1e40af', border: '#bfdbfe' },
 QualityCheck: { bg: '#fef9c3', text: '#854d0e', border: '#fef08a' },
 Ready: { bg: '#d1fae5', text: '#065f46', border: '#bbf7d0' },
 Invoiced: { bg: '#e0e7ff', text: '#3730a3', border: '#c7d2fe' },
...
export function CustomerHistoryDialog({ customerId, customerName, open, onClose }: CustomerHistoryDialogProps) {
 ...
 const jobCards: JobCardDto[] = jobCardsData ?? [];
 ...
 <td style={tdStyle}>{jc.services.length} service{jc.services.length !== 1 ? 's' : ''}</td>
 ...
 >
 {jc.status.replace(/([A-Z])/g, ' $1').trim()}
 </span>
 </td>
